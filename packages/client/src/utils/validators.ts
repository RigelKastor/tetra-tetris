...
ruleMap.set('login', {
    message: 'Логин должен быть от 3 до 20 символов, латиница, может содержать цифры,\
...
ruleMap.set('email', {
    message: 'Латиница, может включать цифры и спецсимволы вроде дефиса, обязательно должна быть «собака» (@)\
...
ruleMap.set('phone', {
    message: 'Телефон должен быть строкой от 10 до 15 символов,\
...
const validate = (field: string, value: string) => {
    ...
    if (!rule)
        return Promise.reject(new Error('unknown rule'))
